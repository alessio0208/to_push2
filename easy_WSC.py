...
def evaluation(grid_option):
	for currentFold in range(1, folds+1):
		...
		training_file = os.path.join(outputpath, outputname  + scenario + '_100S_50SP_15ISP_20IMP_' + str(currentFold) + '.train')
		model_file = os.path.join(outputpath, outputname + scenario + '_100S_50SP_15ISP_20IMP_' + str(currentFold) + '.model')
		...
		#cmd = '{0} -c {1} -g {2} "{3}" "{4}"'.format(svmtrain_exe_q,8192,8, training_file, model_file)
		...
		testing_file = os.path.join(outputpath, outputname + scenario + '_100S_50SP_15ISP_20IMP_' + str(currentFold) + '.test')
		predict_file = os.path.join(outputpath, outputname + scenario + '_100S_50SP_15ISP_20IMP_' + str(currentFold) + '.predict')
	...
	for currentFold in range(1, folds+1):
		fpredict = open(os.path.join(outputpath, outputname + scenario +  '_100S_50SP_15ISP_20IMP_' + str(currentFold) + '.predict'), 'r')
